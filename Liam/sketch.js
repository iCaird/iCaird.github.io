...
function setup() {
  ...
  floor = new Sprite(width / 2, height, width, 10);
  floor.collider = "kinematic";
  floor.bounciness = 1;
  floor.friction = 0;

  ceiling = new Sprite(width / 2, 0, width, 10);
  ceiling.collider = "kinematic";
  ceiling.bounciness = 1;
  ceiling.friction = 0;

  left = new Sprite(0, height / 2, 10, height);
  left.collider = "kinematic";
  left.bounciness = 1;
  left.friction = 0;

  right = new Sprite(width, height / 2, 10, height);
...
function draw() {
  ...
  if (alive == 0) {
    if (first) {
      ...
      endmsg = new Sprite(width / 2, height / 2, width / 2, 34);
